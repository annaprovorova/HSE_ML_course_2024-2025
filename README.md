...
Материалы для самостоятельной работы https://colab.research.google.com/drive/1F9sfm8H0KhO_K6CblgBrP_nojzzH4ddn?usp=sharing
...
2) Сформировать свою выборку для обучения модели которая будет содержать **не менее 100 изображений** с наличием
объекта для распознания и **не менее 100** изображений **без** объекта для распознования. Обучить на собранных данных
...
Материалы для работы на семинаре https://colab.research.google.com/drive/1KlnsCtujeVT9UZoRn9hTomLMaF3RG-Om?usp=sharing

1) Изучите предложенный пример. 
2) Модифицируйте предложенный пример под **свои** пары вопрос-ответ на любую тему. **Сформулируйте не менее 5 своих пар вопросов-ответов.**
...
##### Работа с графикой (часть 2 – нейронные сети на примере YOLO 8)

Материалы для самостоятельной работы https://colab.research.google.com/drive/1VLeJwriQD5C5Ou0N-QVWftFVVejD9oLH?usp=sharing

1) Загрузите и разметьте данные подготовленные для метода HOG (или другие) с использованием сервиса Roboflow (https://roboflow.com) 
2) Дообучите модель YOLO 8 предобученную на данных COCO (https://cocodataset.org). Пример реализации см. в материалах к семинару (в них же есть ссылка на гугл колаб)
3) Подайте на вход модели изображения из тестовой выборки, оцените качество её работы. Объясните метрики качества работы.

*Дополнительные баллы*: 

1.Используйте модель SSD, EfficientDet или другу модель для решения залачи детекции и сравните полученные результаты.
